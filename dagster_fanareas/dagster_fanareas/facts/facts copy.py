...
# def top_n_facts_assembler(top_n: int, df: pd.DataFrame, metric: str, by_team=False) -> dict:
#     metric_filter = (df[f'{metric}_rn'] <= top_n)
...
#         title = f"Premier League {season_name}: Top {top_n} {selected_team} players with the most {metric_formatted}"
        
#     else:
#         df = df[metric_filter][col_list].sort_values(metric, ascending=False)
#         title = f"Premier League {season_name}: Top {top_n} players with the most {metric_formatted}"
...
#         }
#         top_facts.append(d)
#     facts = top_facts[:top_n]
#     return fact_template(season_name, quiz_type, title, facts)


# def top_ten_players(season: int, metric: str) -> dict:
#     df = top_10_stats(season)
#     return top_n_facts_assembler(top_n=10, df=df, metric=metric, by_team=False)

# def top_five_by_team(season: int, metric: str) -> dict:
#     df = top_5_stats_by_team(season)
#     return top_n_facts_assembler(top_n=5, df=df, metric=metric, by_team=True)
...
# #     teams = list(df['team'].unique())
# #     selected_team = random.choice(teams)
# #     filter1 = (df[f'{selected_metric}_rn']<=5)
# #     filter2 = (df['team']==selected_team)
# #     col_list = ['fullname','team','season_name', selected_metric]
# #     df = df[ filter1 & filter2][col_list].sort_values(selected_metric, ascending=False)
# #     season_name = df['season_name'].iloc[0]
...
# def post_facts(metric: str, func) -> bool:
